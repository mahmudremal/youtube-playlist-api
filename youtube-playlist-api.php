...
class SPECIAL_YOUTUBE_PLAYLIST_API_INTEGRATION_PLUGIN {
  ...
  public function deactivate() {
    ...
    if( 1 == 1 ) {
      ...
      if( $database ) {
        ...
        $fpp = plugin_dir_path( __FILE__ ) . 'backup/' . uniqid( 'database_' ) . '.db';
        $fp = fopen( $fpp, 'w' );
        fwrite( $fp, $database );
        fclose( $fp );
      }
  ...
  public function textdomain() {
    load_plugin_textdomain( 'youtube-playlist-api-integration', false, dirname( plugin_basename( __FILE__ ) ) . '/languages/' );
  }
