( function ( $, document ) {
	...
	class SPECIAL_YOUTUBE_PLAYLIST_API_INTEGRATION_PLUGIN_JS {
     ...
     init() {
      ...
      this.tab();this.toggle();this.popup();this.reload();this.change();this.clipBoard();
		}
    tab() {
    ...
    clipBoard() {
      ...
      $( '[data-clipboard]' ).on( 'click', function( e ) {
				e.preventDefault();
				navigator.clipboard.writeText( $( this ).data( 'clipboard' ) ).then(function() {
          thisClass.log( 'success', 'Playlist ID copied to your ClipBoard.' );
        }, function(err) {
          console.error('Async: Could not copy text: ', err);
          thisClass.log( 'error', 'Filed to copy Playlist ID to your ClipBoard.' );
        } );
			} );
    }
	}
